...
def get_all_expenses(search, type, employeeId, page, limit, usr, db: Session):
    ...
    moneyHistorys = db.query(
        ...
        label("liablePerson", func.IF(type == 'salary', func.concat(Employee.firstname, ' ', Employee.lastname), None)),
        label("createdAt", Expense.createdAt),
        label("value", Expense.value),
        label("moneyForm", Moneyform.name),
    ).select_from(Expense)\
        .filter(Expense.type==type)\
        .join(Expense.user).join(User.employee)\
        .join(Expense.moneyForm).join(Expense.employee, worker_alias)
